#!/usr/bin/env python3
"""
Garden7 MongoDB to Supabase Migration Script

이 스크립트는 Garden7의 MongoDB slack_messages 덤프 파일을 
...
def migrate_data():
    ...
    print("=== Garden7 MongoDB to Supabase 마이그레이션 ===")
    print("이 스크립트는 Garden7의 slack_messages 덤프 파일을 Supabase로 마이그레이션합니다.\n")
    ...
    if not os.path.exists(bson_file):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        possible_paths = [
            os.path.join(script_dir, "20250804_mongodb_dump/slack_messages.bson"),
    ...
    schema_name = config['supabase'].get('schema', 'garden7')
